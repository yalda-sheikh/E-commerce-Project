            ...
            public void handle(HttpExchange exchange) throws IOException {
                ...
                if (exchange.getRequestMethod().equalsIgnoreCase("POST")) {
                    ...
                    if (currentCustomer != null) {
                        ...
                        CheckoutResult result = currentCustomer.checkout(discountCode);
...
// ۴. مدیریت درخواست POST (گرفتن محصول جدید از فروشنده فرانت)
            ...
            public static void loadData() {
                try {
                    ...
                    File f3 = new File("product_items.txt");
                    if (!f3.exists()) f3.createNewFile();
                    File f4 = new File("reviews.txt");
                    if (!f4.exists()) f4.createNewFile();
                    File f5 = new File("purchases.txt");
                    if (!f5.exists()) f5.createNewFile();
                    File f6 = new File("new_questions.txt");
